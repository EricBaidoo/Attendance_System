...
try {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) FROM member_roles WHERE status = 'active'");
    $overview_stats['total_members'] = $stmt->fetchColumn();
    
    // Get visitors count
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM visitor_roles WHERE DATE(created_at) BETWEEN ? AND ?");
    ...
    try {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM new_convert_roles WHERE DATE(COALESCE(date_converted, created_at)) BETWEEN ? AND ?");
        $stmt->execute([$start_date, $end_date]);
        $overview_stats['new_converts'] = $stmt->fetchColumn();
    } catch (Exception $e) {
        // Table might not exist
    }
