...
$status_filter_input = $_GET['status'] ?? 'active';
$baptism_filter = $_GET['baptism'] ?? '';
$group_filter = $_GET['group'] ?? '';
$gender_filter = $_GET['gender'] ?? '';
$ministerial_filter = $_GET['ministerial'] ?? '';

$allowed_status_filters = ['active', 'inactive', 'all', ''];
if (!in_array($status_filter_input, $allowed_status_filters, true)) {
    $status_filter_input = 'active';
}
$status_filter = $status_filter_input === 'all' ? '' : $status_filter_input;
$status_filter_ui = $status_filter_input === '' ? 'all' : $status_filter_input;

// Build query filters
$where_conditions = ["p.current_stage = 'member'"];
$params = [];
...
if ($gender_filter) {
    $where_conditions[] = "(LOWER(mr.gender) = LOWER(?) OR mr.gender = ?)";
...
if ($search) {
    $where_conditions[] = "(p.full_name LIKE ? OR p.email LIKE ? OR p.phone LIKE ? OR p.alt_phone LIKE ?)";
    $search_param = "%$search%";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
...
if ($department_filter) {
    $where_conditions[] = "mr.department_id = ?";
    $params[] = $department_filter;
}

if ($baptism_filter) {
    $where_conditions[] = "mr.baptized = ?";
    $params[] = $baptism_filter;
}

if ($group_filter) {
    $where_conditions[] = "mr.congregation_group = ?";
    $params[] = $group_filter;
}

if ($ministerial_filter) {
    if ($ministerial_filter === 'none') {
        $where_conditions[] = "(mr.ministerial_status IS NULL OR mr.ministerial_status = '')";
    } else {
        $where_conditions[] = "mr.ministerial_status = ?";
...
try {
    ...
    $table_check = $pdo->query("SHOW COLUMNS FROM member_roles LIKE 'congregation_group'");
    $has_congregation_group = $table_check->rowCount() > 0;
    $ms_check = $pdo->query("SHOW COLUMNS FROM member_roles LIKE 'ministerial_status'");
    $has_ministerial_status = $ms_check->rowCount() > 0;
    
    if ($has_congregation_group) {
        $stats_sql = "SELECT 
            COUNT(*) as total,
            COUNT(CASE WHEN LOWER(mr.gender) = 'male' THEN 1 END) as male,
            COUNT(CASE WHEN LOWER(mr.gender) = 'female' THEN 1 END) as female,
            COUNT(CASE WHEN mr.baptized = 1 OR mr.baptized = 'yes' THEN 1 END) as baptized,
            COUNT(CASE WHEN mr.congregation_group = 'Adult' THEN 1 END) as adults,
            COUNT(CASE WHEN mr.congregation_group = 'Youth' THEN 1 END) as youths,
            COUNT(CASE WHEN mr.congregation_group = 'Teen' THEN 1 END) as teens,
            COUNT(CASE WHEN mr.congregation_group = 'Children' THEN 1 END) as children";
    } else {
        $stats_sql = "SELECT 
            COUNT(*) as total,
            COUNT(CASE WHEN LOWER(mr.gender) = 'male' THEN 1 END) as male,
    ...
    if ($has_ministerial_status) {
        $stats_sql .= ",
            COUNT(CASE WHEN mr.ministerial_status = 'Levite' THEN 1 END) as ms_levite,
            COUNT(CASE WHEN mr.ministerial_status = 'Shepherd' THEN 1 END) as ms_shepherd,
            COUNT(CASE WHEN mr.ministerial_status = 'Minister' THEN 1 END) as ms_minister,
            COUNT(CASE WHEN mr.ministerial_status = 'Junior Pastor' THEN 1 END) as ms_jp,
            COUNT(CASE WHEN mr.ministerial_status = 'Senior Pastor' THEN 1 END) as ms_sp,
            COUNT(CASE WHEN mr.ministerial_status = 'General Overseer' THEN 1 END) as ms_go,
            COUNT(CASE WHEN mr.ministerial_status IS NULL OR mr.ministerial_status = '' THEN 1 END) as ms_none";
    } else {
        $stats_sql .= ", 0 as ms_levite, 0 as ms_shepherd, 0 as ms_minister, 0 as ms_jp, 0 as ms_sp, 0 as ms_go, 0 as ms_none";
    }

    $stats_sql .= " FROM member_roles mr JOIN people p ON mr.person_id = p.id $where_clause";
...
$members_sql = "SELECT mr.id, p.full_name AS name, p.email, p.phone, p.alt_phone AS phone2, d.name as department_name, mr.gender, mr.baptized, mr.congregation_group, mr.ministerial_status, mr.status, mr.date_joined
...
$count_sql = "SELECT COUNT(*) as total FROM member_roles mr JOIN people p ON mr.person_id = p.id $where_clause";
            ...
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-2 col-sm-6">
                    ...
                    <select name="status" class="form-select">
                        <option value="all" <?php echo $status_filter_ui === 'all' ? 'selected' : ''; ?>>All</option>
                        <option value="active" <?php echo $status_filter_ui === 'active' ? 'selected' : ''; ?>>Active</option>
                        <option value="inactive" <?php echo $status_filter_ui === 'inactive' ? 'selected' : ''; ?>>Inactive</option>
                    </select>
                </div>
                <div class="col-md-2 col-sm-6">
