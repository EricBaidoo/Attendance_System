...
$current_member = null;
if ($user_role == 'member') {
    $member_sql = "SELECT mr.* FROM member_roles mr JOIN people p ON mr.person_id = p.id WHERE p.email = ? OR p.full_name = ? LIMIT 1";
...
if ($selected_session) {
    $members_sql = "SELECT mr.id, p.full_name AS name, p.email, p.phone, d.name as department_name, 
                    ...
                    LEFT JOIN attendance a ON mr.id = a.member_id 
                        AND a.session_id = ? 
